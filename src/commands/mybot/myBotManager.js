...
async function getNextBotPort(currentBots) {
    let maxPort = 3456;
    for (const bot of currentBots) {
        if (bot.config && bot.config.webPort && bot.config.webPort > maxPort) {
            maxPort = bot.config.webPort;
        }
    }
    return maxPort + 1;
}
...
    await ensureDirectories();
    ...
    const webPort = await getNextBotPort(currentBots);
...
    try {
        ...
        const webPort = await initializeBotFiles(botId, imei, cookie, null, null, botList);
...
        }
        ...
        const pm2Command = `pm2 start ${indexPath} --name "${processName}" -- ${botId} ${webPort}`;
        console.log(`[MyBot] 🚀 Khởi chạy PM2: ${pm2Command}`);
...
        const status = formatPm2Status(realStatus, botConfig.isRunning);
        const webPort = botConfig.webPort || 3456;
...
            const expireInfo = formatRemainingTime(bot.config.expiresAt);
            const webPort = bot.config.webPort || 3456;
...
        }
        ...
        const webPort = botConfig.webPort || 3456;
        const pm2Command = `pm2 start ${processName} -- ${botId} ${webPort}`;
...
        }
        ...
        const webPort = botConfig.webPort || 3456;
        const pm2Command = `pm2 restart ${processName} -- ${botId} ${webPort}`;
        
        await execAsync(pm2Command);
