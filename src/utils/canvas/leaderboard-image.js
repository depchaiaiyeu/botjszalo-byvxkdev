...
export async function drawLeaderboardImage(topUsers, isToday, targetUser, currentUserUid, rankInfo) {
  ...
  if (targetUser) {
    ...
    const boxHeight = 220;
    const boxY = (totalHeight - boxHeight) / 2 - 20;
    
    const bgGradient = ctx.createLinearGradient(PADDING, boxY, WIDTH - PADDING, boxY + boxHeight);
    bgGradient.addColorStop(0, 'rgba(90, 110, 235, 0.35)');
    bgGradient.addColorStop(1, 'rgba(167, 182, 255, 0.15)');
    ctx.fillStyle = bgGradient;
    drawRoundedRect(PADDING, boxY, WIDTH - PADDING * 2, boxHeight, RADIUS);
    ctx.fill();
    
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 32px "BeVietnamPro"';
    ctx.textAlign = 'center';
    ctx.fillText(user.UserName, WIDTH / 2, boxY + 50);
    
    ctx.font = '24px "BeVietnamPro"';
    ctx.fillStyle = '#94a3b8';
    ctx.fillText(`${count} tin nhắn ${isToday ? "trong hôm nay" : ""}`, WIDTH / 2, boxY + 90);
  } else {
...
export async function drawTopChatImage(topUsers, lastMessageTime, targetUser = false) {
  const WIDTH = 800;
  const HEADER_HEIGHT_TOP = targetUser ? 120 : 150;
  const HEADER_HEIGHT_TABLE = 60;
  ...
  if (targetUser) {
    const boxHeight = 220;
    totalHeight = 500;
    currentY = (totalHeight - boxHeight) / 2;
  } else {
    const totalRowsHeight = listLength * (ROW_HEIGHT + GAP);
    totalHeight = HEADER_HEIGHT_TOP + HEADER_HEIGHT_TABLE + totalRowsHeight + 50;
    currentY = HEADER_HEIGHT_TOP;
  }
  ...
  const titleText = "🏆 THỐNG KÊ TƯƠNG TÁC 🏆";
  ctx.textAlign = 'center';
  ctx.fillStyle = cv.getRandomGradient(ctx, WIDTH);
  ctx.font = 'bold 48px "BeVietnamPro", Arial';
  ctx.fillText(titleText, WIDTH / 2, 65);
  ...
  if (targetUser) {
    ...
    ctx.font = 'bold 32px "BeVietnamPro"';
    ctx.textAlign = 'center';
    ctx.fillText(user.UserName, WIDTH / 2, currentY + 50);
    
    ctx.font = '24px "BeVietnamPro"';
    ctx.fillStyle = '#94a3b8';
    ctx.fillText(`${count} tin nhắn`, WIDTH / 2, currentY + 90);
  } else {
    const HEADER_Y_TABLE = currentY + HEADER_HEIGHT_TABLE / 2 + 5;
    ...
    for (let i = 0; i < listLength; i++) {
      ...
      ctx.font = '24px "BeVietnamPro"';
      ctx.textAlign = 'left';
      ctx.fillText(user.UserName, PADDING + 130, y + 35);
  ...
  ctx.fillStyle = '#94a3b8';
  ctx.fillText(`Cập nhật: ${lastMessageTime}`, WIDTH / 2, totalHeight - 20);
