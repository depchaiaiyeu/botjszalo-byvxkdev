...
    const bgGradient = ctx.createLinearGradient(PADDING, currentY, WIDTH - PADDING, currentY + TARGET_USER_BOX_HEIGHT);
    bgGradient.addColorStop(0, 'rgba(90, 110, 235, 0.35)');
    bgGradient.addColorStop(1, 'rgba(167, 182, 255, 0.15)');
    ctx.fillStyle = bgGradient;
    drawRoundedRect(PADDING, currentY, WIDTH - PADDING * 2, TARGET_USER_BOX_HEIGHT, RADIUS);
    ctx.fill();
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 36px "BeVietnamPro"';
    ctx.textAlign = 'center';
    const detailText = rank !== "N/A" ? `#${rank} - ${user.UserName}: ${count} ${isToday ? "(Hôm nay)" : "(Tổng)"}` : `${user.UserName}: ${count} ${isToday ? "(Hôm nay)" : "(Tổng)"}`;
    ctx.fillText(detailText, WIDTH / 2, currentY + TARGET_USER_BOX_HEIGHT / 2 + 10);
    currentY += TARGET_USER_BOX_HEIGHT;
  } else {
